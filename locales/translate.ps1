...
Write-Host "=== Cleaning old language JSON files ===" -ForegroundColor Cyan

foreach ($lang in $targetLangs) {
    $file = Join-Path $PWD.Path "$lang.json"
    if (Test-Path $file) {
        Remove-Item $file -Force
    }
}

if (!(Test-Path "en.json")) {
    Write-Host "❌ en.json not found in this folder." -ForegroundColor Red
    exit
}
...
Write-Host "`n=== Creating optimized translator ===" -ForegroundColor Cyan
...
[System.IO.File]::WriteAllText((Join-Path $PWD.Path "fast_translator.py"),$pythonCode)

Write-Host "`n=== Installing dependency ===" -ForegroundColor Cyan
python -m pip install deep-translator -q

Write-Host "`n=== Running translation ===" -ForegroundColor Cyan
python fast_translator.py
...
Write-Host "`n🎉 All language files successfully generated!" -ForegroundColor Green